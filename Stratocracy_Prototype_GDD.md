...
and Driver, each as `X.h` and `X.good.cpp`, beside `StratRules.Build.cs`, which is
...
purpose. `StratRules` contains **no engine headers, no UObject, no third-party
...
state because they are a function of the log, and both are hashed. **This hash is still
